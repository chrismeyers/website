...
Future main() async {
  ...
  test("PUT /account/password returns 200 - password changed", () async {
    ...
    TestResponse response = await harness.publicAgent
        .put("/account/password", body: {"password": newPassword});

    // Make sure PUT /account/password succeeds
    expectResponse(response, 200, body: {"id": 1, "username": username});

    // Try logging in with the old password
    harness.publicAgent.headers['authorization'] =
        "Basic ${base64.encode("${harness.client}:".codeUnits)}";
    final TestRequest request = harness.publicAgent.request("/auth/token")
      ..encodeBody = false
      ..body = utf8.encode(
          "username=$username&password=$oldPassword&grant_type=password")
    ...
    request.body = utf8
        .encode("username=$username&password=$newPassword&grant_type=password");
    response = await request.post();
