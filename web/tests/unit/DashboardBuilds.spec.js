...
describe('DashboardBuilds', () => {
  let wrapper;

  beforeEach(() => {
    wrapper = mount(DashboardBuilds);
  });
  ...
  it('renders schema select fields correctly', async () => {
    ...
    await wrapper.setData({ schema, images, selected: { image: [] } });
  ...
  it('selected value is correct', async () => {
    ...
    expect(wrapper.findAll('option').at(0).element.selected).toBe(false);
